...
@WebServlet("/manageCat")
@MultipartConfig
public class manageCategoryServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        request.setCharacterEncoding("UTF-8");
        System.out.println("manageCat后端处理中");
        // 获取参数
        ...
        String opt = request.getParameter("opt"); // 新增或修改标识符
        // 获取文件部分
        ...
        if (opt.equals("new")) {
            categoryFeedbackModel = ProcessCategoryService.processUpload(name, imageBytes, albumId, age, variety, gender,
                    appearance, neutered, healthy, address, region, other, isAdmin);
        } else if (opt.equals("renew")) {
            categoryFeedbackModel = ProcessCategoryService.processRenew(oldName, category, isAdmin);
        } else {
            System.out.println("出错");
            categoryFeedbackModel.setCode(0);
            categoryFeedbackModel.setMsg("服务器预处理出错");
