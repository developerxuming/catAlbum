...
<div class="container mt-5">
    <div class="row justify-content-center">
        <div class="col-md-6 col-sm-12">
            <div class="card">
                <div class="card-body">
                    <form method="post" id="renewCatForm" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            <label for="image">如图片不需要修改，可不选择图片，非必填</label>
...
<script>
    $(document).ready(function() {
        ...
        $.get("getCategoryByName" + (name ? '?name=' + name : ''), function(data) {
            ...
            // // 构造一个File对象
            // let file = new File([blob], "oldImage.jpg", {type: "image/jpeg"});
            // // 创建一个新的 FileList 对象，将文件添加到其中
            // let fileList = new DataTransfer();
            // fileList.items.add(file);
            // // 将 FileList 对象设置为文件上传字段的值
        ...
        $('#renewCatForm').submit(function(event) {
            ...
            let formData = new FormData($('#renewCatForm')[0]);
            console.log("表单数据：", formData);
            console.log("新名字为：" +formData.get('name'))
            console.log("旧名字为：" +formData.get('oldName'))
            console.log("年龄为："+formData.get('age'))
            console.log(formData.get('image'))
            $.ajax({
                url: 'renewCat', // 修改为你的服务器端处理上传的URL
                method: `POST`,
                data: formData,
                processData: false,
                contentType: false,
